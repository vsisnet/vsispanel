...
- Khi tạo commit, đảm bảo các file trên KHÔNG được stage (`git add`)

## Migration Module (Updated 2026-02-27)

### PleskMigrator - Full Plesk Migration
- Rewrote PleskMigrator with complete Plesk-specific logic (not delegating to SshMigrator)
- Handles Plesk subdomain path resolution (subdomains under parent domain dirs)
- WordPress auto-detection via wp-config.php scanning
- Database dumps using Plesk admin auth: `MYSQL_PWD=$(cat /etc/psa/.psa.shadow) mysql -u admin`
- Creates databases via VsisPanel DatabaseService (createDatabase + createDatabaseUser + grantAccess)
- Auto-updates wp-config.php with new DB credentials after migration
- Discovery shows WP badge and disk size per domain
