<template>
  <VModal :show="show" :title="$t('websites.addDomain')" size="lg" @close="handleClose">
    <!-- Progress Steps -->
    <div class="mb-8">
    ...
    <div class="min-h-[200px]">
      ...
      <div v-if="currentStep === 2" class="space-y-4">
        ...
        <label class="flex items-center p-4 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          <div class="ml-4">
            <span class="font-medium text-gray-900 dark:text-white">
              {{ $t('websites.wizard.autoSsl') }}
            </span>
            <p class="text-sm text-gray-500 dark:text-gray-400">
              {{ $t('websites.wizard.autoSslDesc') }}
            </p>
          </div>
        </label>
        ...
        <label class="flex items-center p-4 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          <div class="ml-4">
            <span class="font-medium text-gray-900 dark:text-white">
              {{ $t('websites.wizard.createDns') }}
            </span>
            <p class="text-sm text-gray-500 dark:text-gray-400">
              {{ $t('websites.wizard.createDnsDesc') }}
            </p>
          </div>
        </label>
        ...
        <label class="flex items-center p-4 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          <div class="ml-4">
            <span class="font-medium text-gray-900 dark:text-white">
              {{ $t('websites.wizard.createDatabase') }}
            </span>
            <p class="text-sm text-gray-500 dark:text-gray-400">
              {{ $t('websites.wizard.createDatabaseDesc') }}
            </p>
          </div>
        </label>
      </div>
        ...
        <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
          ...
          <dl class="space-y-3">
            ...
            <div class="flex justify-between">
              ...
              <dd>
                <VBadge :variant="form.auto_ssl ? 'success' : 'secondary'" size="sm">
                  {{ form.auto_ssl ? $t('common.yes') : $t('common.no') }}
                </VBadge>
              </dd>
            </div>
            <div class="flex justify-between">
              ...
              <dd>
                <VBadge :variant="form.create_dns ? 'success' : 'secondary'" size="sm">
                  {{ form.create_dns ? $t('common.yes') : $t('common.no') }}
                </VBadge>
              </dd>
            </div>
            <div class="flex justify-between">
              ...
              <dd>
                <VBadge :variant="form.create_database ? 'success' : 'secondary'" size="sm">
                  {{ form.create_database ? $t('common.yes') : $t('common.no') }}
                </VBadge>
              </dd>
            </div>
          </dl>
    ...
    <div class="mt-8 flex justify-between">
      ...
      <div class="flex space-x-3">
        <VButton variant="secondary" :disabled="creating" @click="handleClose">
          {{ $t('common.cancel') }}
        </VButton>
        <VButton
...
const canProceed = computed(() => {
  if (currentStep.value === 0) {
    return form.value.name && !errors.value.name
  }
  return true
})
...
async function createDomain() {
  creating.value = true
  creatingMessage.value = t('websites.wizard.creatingDomain')
  ...
  try {
    ...
    // or through separate API calls here
    ...
    // Navigate to the new domain detail page
    router.push({ name: 'domain-detail', params: { id: domain.id } })
  } catch (err) {
    appStore.showToast({
      type: 'error',
      message: err.response?.data?.error?.message || t('websites.createError')
    })
  } finally {
    creating.value = false
  }
